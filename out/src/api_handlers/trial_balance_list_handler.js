"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const axios_1 = require("axios");
class TrialBalanceListHandler {
    constructor(config) {
        this.config = config;
    }
    handle(expressRequest, expressResponse) {
        return __awaiter(this, void 0, void 0, function* () {
            let url = this.config.api + '/api/trialbalance';
            axios_1.default.get(url, {
                headers: { 'Authorization': 'Bearer ' + this.config.token }
            })
                .then(function (axiosResponse) {
                this.handleSuccessResponse(expressResponse, axiosResponse.status, axiosResponse.data);
            })
                .catch(function (axiosError) {
                let err = { errCode: axiosError.code, message: axiosError.message };
                this.handleErrorResponse(expressResponse, axiosError.response.status, err);
            });
        });
    }
    handleSuccessResponse(expressResponse, status, data) {
        expressResponse.json(data);
        expressResponse.status(status);
        expressResponse.end();
    }
    handleErrorResponse(expressResponse, status, err) {
        expressResponse.json({ errorInfo: err });
        expressResponse.status(status);
        expressResponse.end();
    }
}
exports.TrialBalanceListHandler = TrialBalanceListHandler;
//# sourceMappingURL=trial_balance_list_handler.js.map